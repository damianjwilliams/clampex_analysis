"""
Apply the fast/slow classifier (train.py) to every event miniML already
detected in a recording (detect.py's *_miniML_individual.csv), labeling
each one fast or slow.
...
used in training are computed (features.extract_features: amplitude,
10-90% rise time, half-decay time, exponential decay tau, charge), and the
trained sklearn pipeline predicts a label + probability.
...
Output (next to the source .abf):
    ...
    <name>_miniML_classified.png    raw trace with each event marked
                                     fast/slow (colors match style.py)

Usage
-----
    python -m sepsc.classify path\\to\\recording.abf
    python -m sepsc.classify recording.abf --model path\\to\\model.joblib
"""
...
from .style import COLORS, MARKERS, SURFACE, TRACE
...
def main(argv=None):
    ...
    if not os.path.exists(csv_path):
        p.error(f"{csv_path!r} not found -- run `python -m sepsc detect` first")
    if not os.path.exists(model_path):
        p.error(f"{model_path!r} not found -- run `python -m sepsc train` first, or pass --model")
