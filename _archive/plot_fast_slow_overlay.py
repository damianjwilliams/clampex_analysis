# ARCHIVED 2026-08-27: superseded by sepsc/overlay.py -- see
# sepsc/__init__.py for the combined pipeline (python -m sepsc overlay ...).
...
labeled fast and slow sEPSC (from label_sEPSC_training_events_gui.py), so
the two populations' kinetics can be compared visually.

Every event is:
  * peak-located the same way train_sEPSC_classifier.py does (a lightly
    smoothed copy of the window, anchored to the known peak offset -- see
    that script's locate_peak_and_baseline), so this plot and the trained
    classifier agree on where each event's peak actually is;
...
Usage
-----
    python plot_fast_slow_overlay.py path\\to\\recording.abf
    python plot_fast_slow_overlay.py rec1.abf rec2.abf --out combined_overlay.png
"""
...
from train_sEPSC_classifier import locate_peak_and_baseline

# validated CVD-safe categorical pair (dataviz skill reference palette,
# slots 1-2) -- matches the blue/orange already used by the labeling GUI
