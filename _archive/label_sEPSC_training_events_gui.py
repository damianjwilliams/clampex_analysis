# ARCHIVED 2026-08-27: superseded by sepsc/label.py -- see sepsc/__init__.py
# for the combined pipeline (python -m sepsc label ...). Kept here only for
...
recording (use review_sEPSCs_gui.py to vet miniML's own detections instead).
...
Usage
-----
    python label_sEPSC_training_events_gui.py path\\to\\recording.abf
    python label_sEPSC_training_events_gui.py recording.abf --chunk-s 3 --min-amp 8
...
LABELS = {1: "fast", 3: "slow"}  # matplotlib mouse button codes: 1=left, 3=right
...
def safe_callback(fn):
    """Print (flushed) instead of silently dying inside a Tk/matplotlib
    callback -- an uncaught exception there can leave the window looking
    frozen with nothing ever printed to explain why."""
    @functools.wraps(fn)
...
class LabelSession:
    ...
    def on_click(self, event):
        if self._busy or event.inaxes != self.ax or event.button not in LABELS:
            return
        label = LABELS[event.button]
...
def main(argv=None):
    ...
    p.add_argument("--smooth-samples", type=int, default=15,
                   help="boxcar smoothing window (samples) used only for peak/onset/decay "
                        ...
                        "detect_sEPSCs_miniML.py's convolve_win)")
